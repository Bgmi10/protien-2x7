    ...
    <div className="pt-16 sm:pt-20 pb-8 sm:pb-16 min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-[12px] sm:text-lg lg:text-xl text-gray-600 px-2">Real transformations from our satisfied customers</p>
        </motion.div>
          ...
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div className="flex items-center space-x-2 sm:space-x-3 mb-2 sm:mb-3 lg:mb-4">
                ...
                />
                <div>
                  <h3 className="font-semibold text-gray-900 text-[10px] sm:text-sm lg:text-base">{testimonial.name}</h3>
                  <p className="text-gray-600 text-[8px] sm:text-xs lg:text-sm">{testimonial.role}</p>
                </div>
              </div>
              ...
              <p className="text-gray-700 leading-relaxed text-[8px] sm:text-xs lg:text-sm">"{testimonial.content}"</p>
            </motion.div>
          ))}
        </div>
      </div>
